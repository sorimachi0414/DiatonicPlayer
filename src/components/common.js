...
//コードを鳴らすタイミングを設定するチェックボックス
export const ChordChopperCheckBox = (props) => {
  return (
    <input className="form-check-input m-05 p-sm-2 mx-sm-1" type="checkbox" checked={props.checked} value={props.value}
    ...
    <Row className="p-2 px-3">
      <Col xs={3} sm={3} md={3} className="p-0 m-0">
        <button className="btn btn-outline-primary p-0 w-100 h-100" value={props.value}
                onClick={props.onClickN}>{"<"}</button>
      </Col>
      <Col xs={6} sm={6} md={6} className="p-0 m-0 ">
        <button className={props.color} value={props.value} onClick={props.onClickP}><span
          className="fs-2"> {props.value}</span></button>
      </Col>
      <Col xs={3} sm={3} md={3} className="p-0 m-0">
        <button className="btn btn-outline-primary p-0 w-100 h-100" value={props.value}
                onClick={props.onClickP}>{">"}</button>
      </Col>
    </Row>
  )
}
...
export const DropDownSelector = (props) => {
  ...
  return (
    <select
      ...
      className="form-select w-100" onChange={(e) => props.change(e.target.value)}>
      {options}
    </select>
  )
}
  ...
  render() {
    ...
    return (
      ...
      <select
        ...
        onChange={(e) => this.props.onChange(this.props.boxNum, e.target.value)}>
        {options}
      </select>
    )
  }
